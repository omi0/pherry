...
import { MIRROR_SNAPSHOT, PROTOCOL_VERSION, PTY_STREAM, SESSION_INPUT } from '@pherry/protocol'
...
/** Build the controller `Hello` control frame for a raw test client. */
export function controllerHello(
  capabilities: readonly string[] = CONTROLLER_CAPS,
  protocol: number = PROTOCOL_VERSION,
): ChannelFrame {
  const hello = { role: 'controller', protocol, capabilities: [...capabilities], publicKey: '' }
